...
NoobPlsDontKillAI* makeAIShellFromInput(){

	NoobPlsDontKillAI* shell = NULL;
		...
		if (input == end){
			...
			for (int col =0; col<colCount; col++){
				for (int row =0; row<rowCount; row++){
					...
					if (gameState[col][row] != NoobPlsDontKillAI::NO_PIECE)
					{
			...
			NoobPlsDontKillAI* shell = new NoobPlsDontKillAI(colCount, rowCount, gravity, gameState, m);
...
int main() {
	...
	NoobPlsDontKillAI *shell = makeAIShellFromInput();
